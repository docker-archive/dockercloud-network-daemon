...
if [ "${WEAVE_LAUNCH}" = "**None**" ]; then
    ...
    if [ ! -z "${WEAVE_PASSWORD}" ]; then
        ...
        /weave --local launch-router --ipalloc-range=10.128.0.0/10 --init-peer-count ${TUTUM_PEER_COUNT} --password ${WEAVE_PASSWORD} ${WEAVE_LAUNCH} || true
    else
        echo "!! WARNING: No \$WEAVE_PASSWORD set!"
        echo "=> Running: weave launch ${WEAVE_LAUNCH}"
        echo "=> Peer count: ${TUTUM_PEER_COUNT}"
        /weave --local launch-router --ipalloc-range=10.128.0.0/10 --no-discovery --init-peer-count ${TUTUM_PEER_COUNT} ${WEAVE_LAUNCH} || true
    fi
    sleep 2
fi
