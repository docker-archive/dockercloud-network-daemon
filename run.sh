...
0-9]{1,3}/[0-9]+)" | tr '\n' ',' | head -c -1)
    if [ ! -z "${TUTUM_PRIVATE_CIDR}" ]; then
        PRIVATE_SUBNETS="${PRIVATE_SUBNETS},${TUTUM_PRIVATE_CIDR}"
    fi
    echo "=> Marking the following private subnets as trusted (unencrypted): ${PRIVATE_SUBNETS}"

    if [ ! -z "${WEAVE_PASSWORD}" ]; then
        echo "=> Running: weave launch -password XXXXXX ${WEAVE_LAUNCH}"
        echo "=> Peer count: ${TUTUM_PEER_COUNT}"
        /weave --local launch-router --ipalloc-range=10.128.0.0/10 --trusted-subnets=${PRIVATE_SUBNETS} --no-dns --no-discovery --init-peer-count=${TUTUM_PEER_COUNT} --password=${WEAVE_PASSWORD} ${WEAVE_LAUNCH} || true
    else
        echo "!! WARNING: No \$WEAVE_PASSWORD set!"
        echo "=> Running: weave launch ${WEAVE_LAUNCH}"
        echo "=> Peer count: ${TUTUM_PEER_COUNT}"
        /weave --local launch-router --ipalloc-range=10.128.0.0/10 --trusted-subnets=${PRIVATE_SUBNETS} --no-dns --no-discovery --init-peer-count=${TUTUM_PEER_COUNT} ${WEAVE_LAUNCH} || true
    fi
    sleep 2
fi
