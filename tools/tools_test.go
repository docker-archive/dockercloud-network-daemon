...
func Test_compareArrays(t *testing.T) {
	...
	nodeIps := []string{`10.0.0.1`, `10.0.0.2`, `10.0.0.3`, `10.0.0.4`}
	nodeIps2 := []string{`10.0.0.1`, `10.0.0.3`, `10.0.0.4`}
	peerIps := []string{`10.0.0.2`}

	diff1 = CompareArrays(nodeIps, peerIps, diff1)
	...
	diff2 = CompareArrays(peerIps, nodeIps2, diff2)
...
func Test_removeDuplicates(t *testing.T) {
	...
	aWithoutDuplicates := []string{"192.168.130.23", "192.168.130.24", "192.168.130.22"}

	a = RemoveDuplicates(a)

	if !testEq(a, aWithoutDuplicates) {
		t.Error("Unexpected node ips list")
	}
}
